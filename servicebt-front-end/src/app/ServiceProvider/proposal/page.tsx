...
const JobProposal = () => {
  ...
  const [selectedSkills, setSelectedSkills] = useState<{ value: string; label: string }[]>([]); // Selected skills
  ...
  useEffect(() => {
    const fetchSkills = async () => {
      try {
        ...
        const formattedSkills = response.data.map((skill: { name: string; }) => ({
          value: skill.name,
          label: skill.name
        }));
  ...
  const handleSubmit = async (e: { preventDefault: () => void; }) => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        router.push("pages/ServiceProvider/proposal-list");
      }
    } catch (error) {
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* Skills Selection with Typeahead Dropdown */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                ...
                <Select
                  isMulti
                  options={skills} // Preloaded skills from backend
                  value={selectedSkills}
                  onChange={(newValue) => setSelectedSkills(newValue as { value: string; label: string }[])}
